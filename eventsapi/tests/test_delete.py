...
async def test_success_delete(event):
    async with AsyncClient(app=main.app, base_url='http://localhost:8000') as ac:
        response = await ac.delete(f'/events/{str(event.id)}')
        assert response.status_code == 200


@pytest.mark.asyncio
@pytest.mark.usefixtures('event')
async def test_not_found_delete():
    async with AsyncClient(app=main.app, base_url='http://localhost:8000') as ac:
        response = await ac.delete(f'/events/{str(uuid.uuid4())}')
